import isValidGeo, { parseCoordinates } from '../geolocation/isValidGeo';
...
describe('parseCoordinates', () => {
  ...
  test('invalid input with too many parts', () => {
    expect(() => parseCoordinates('51.50851, −0.12572, 123.456')).toThrow('Некорректный формат ввода');
  });

  test('invalid input with incorrect format', () => {
    expect(() => parseCoordinates('51.50851: −0.12572')).toThrow('Некорректный формат ввода');
  });

  test('invalid input with non-numeric values', () => {
    expect(() => parseCoordinates('51.50851, abc')).toThrow('Некорректные значения широты и долготы');
  });
});
